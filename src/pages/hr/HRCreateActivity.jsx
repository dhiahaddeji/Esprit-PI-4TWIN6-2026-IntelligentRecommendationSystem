    ...
    <div style={{ padding: 18, maxWidth: 920 }}>
      ...
      <form onSubmit={onSubmit} style={{ marginTop: 16, display: "grid", gap: 12 }}>
        ...
        <div style={{
          ...
          <div style={{ fontWeight: 800, color: "#1A1A2A", fontSize: 14, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 7, marginBottom: 8 }}>
            <button type="button" onClick={() => setCatalogMode(true)} style={{
              ...
              background: catalogMode ? "#EEF7FA" : "#E4E4F0",
          ...
          {catalogMode ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, maxHeight: 130, overflowY: "auto" }}>
              {filteredCatalog.length === 0
                ? <span style={{ color: "#638899", fontSize: 12 }}>Aucun item disponible dans cette catégorie.</span>
...
function Field({ label, children }) {
  return (
    <label style={{ display: "grid", gap: 6 }}>
      <span style={{ fontWeight: 800, color: "#1A1A2A" }}>{label}</span>
      {children}
    </label>
  );
}
